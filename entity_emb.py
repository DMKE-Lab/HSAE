...
class Ent_time_emb(torch.nn.Module):
    def __init__(self, dataset, params):
        super(Ent_time_emb, self).__init__()
        self.dataset = dataset
        self.params = params
        
        self.tim_embs_f = nn.Embedding(dataset.numTime(),params.emb_dim).cuda()

        self.h_map_emb = nn.Embedding(dataset.numEnt()+1, params.emb_dim).cuda()
        self.t_map_emb = nn.Embedding(dataset.numEnt()+1, params.emb_dim).cuda()
        self.rel_emb_h = nn.Embedding(dataset.numRel(), params.emb_dim).cuda()
        self.rel_emb_t = nn.Embedding(dataset.numRel(), params.emb_dim).cuda()
        self.ent_emb_q = nn.Embedding(dataset.numEnt(), params.emb_dim).cuda()
        ...
        nn.init.xavier_uniform_(self.ent_emb_q.weight)
    ...
    def getRelEmbeddings(self,heads, rels, tails, hiss, timeid,ent_type,train_or_test):
        ...
        if ent_type == "subs":
            if train_or_test == "train":
                ...
                ents_emb, rels_emb, rel_set_emb = self.ent_emb_q(ents_for_rel), self.rel_emb_h(rels_his), self.h_map_emb(hiss)
            ...
            if train_or_test == "train": 
                ...
                ents_emb, rels_emb, rel_set_emb = self.ent_emb_q(ents_for_rel), self.rel_emb_t(rels_his), self.t_map_emb(hiss)
                pre_rel_emb = self.get_rel_embedds(ents_emb, rels_emb, rel_set_emb, timeid, hiss)
                pre_rel_emb = pre_rel_emb.unsqueeze(1)
                pre_rel_emb = pre_rel_emb.repeat(1, (1 + self.params.neg_ratio), 1).contiguous().view(-1, self.params.emb_dim)
            else:
    ...
    def get_mask(self, his):
        ...
        idx, len_q = self.dataset.numEnt(), 1
    ...
    def make_his_emb(self, ents, times, rels_emb, rel_set_emb):
        ...
        rels_new = rels_emb.expand(batch_size, size, dim)

        times_new = times.expand(batch_size, size, dim)
        
        his_set_emb = torch.sum(rel_set_emb * rels_new, dim=2, keepdim=True) * times_new
        
        que_rels = torch.sum(ents * rels_emb, dim=2, keepdim=True) * times
