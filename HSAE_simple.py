...
class HSAE_simple(torch.nn.Module):
    def __init__(self, dataset, params):
        super(HSAE_simple, self).__init__()
    ...
    def forward(self, batch1=None, batch2=None, train_or_test="train", ent_type="subs"):
        if batch2 == None:
            ...
            scores = T_embs1*((h_embs1 * r_embs1) * t_embs1 + (h_embs2 * r_embs2) * t_embs2) / 2.0
        else:

            h_embs1, r_embs1, t_embs1, h_embs2, r_embs2, t_embs2 ,T_embs1= self.getEmbeddings(batch1, "objs", train_or_test)
            scores1 = T_embs1*((h_embs1 * r_embs1) * t_embs1 + (h_embs2 * r_embs2) * t_embs2) / 2.0
            h_embs3, r_embs3, t_embs3, h_embs4, r_embs4, t_embs4 ,T_embs2= self.getEmbeddings(batch2, "subs", train_or_test)
            scores2 = T_embs2*((h_embs3 * r_embs3) * t_embs3 + (h_embs4 * r_embs4) * t_embs4) / 2.0
